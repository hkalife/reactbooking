...
describe("Booking - Add booking", () => {
  ...
  it("should display errors - check-out before check-in date (create/edit)", () => {
    cy.get('[data-testid="checkin__field"]').type(
      format(addDays(new Date(), 2), "yyyy-MM-dd")
    );
    cy.get('[data-testid="checkout__field"]').type(
      format(new Date(), "yyyy-MM-dd")
    );
    cy.get('[data-testid="dates__confirm--btn"]').click();
    ...
    cy.get('[data-testid="edit__checkin--0"]').type(
      format(addDays(new Date(), 0), "yyyy-MM-dd")
    );
    cy.get('[data-testid="edit__checkout--0"]').type(
      format(addDays(new Date(), 2), "yyyy-MM-dd")
    );
    cy.get('[data-testid="confirm__edit--0"]').click();
  });
